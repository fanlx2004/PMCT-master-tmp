...
class Vehicle(object):
    ...
    def __init__(self, id, controller, observation_method=Observation, routeID=None, simulator=None, initial_speed=None, initial_position=None, initial_lane_id=None, initial_heading=None):
    ...
    def act(self, action):
        ...
        if action["lateral"] == "central":
            ...
            print(f"time:{self.simulator.current_time_steps * 0.1}:")
            print(f"Vehicle {self.id} is in lane {traci.vehicle.getLaneID(self.id)} and change lane to {action['lateral']}")
