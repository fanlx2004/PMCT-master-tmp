...
class Simulator(object):    
    ...
    def start(self):
        ...
        if self.sumo_control_state:
            ...
            if conf.simulation_config["gui_flag"]:
                sumoCmd[0] = "sumo-gui"  # Force GUI mode
                sumoCmd.extend([
                "--start",  # Auto-start simulation
                "--delay", str(getattr(self, 'gui_delay', 100)),  # ms per step
                 # Close GUI when done
            ])
    ...
    # @profile
    def soft_run(self, AV_info=None, BV_info=None):
        ...
        if AV_info is not None and BV_info is not None:
            self.env.initialize(AV_info, BV_info)
        else:
            self.env.initialize()
    ...
    @silence_output
    def sample_run(self, AV_info=None, BV_info=None):
        self.split_run_flag = True
        self.start()
        self.soft_run(AV_info, BV_info)
        info_final = self.env.info_extractor.episode_log["av_obs"]
        self.stop()
        return info_final
    ...
    def _add_vehicle_to_sumo (self, v, typeID='DEFAULT_VEHTYPE', initial_position=None, initial_lane_id=None, depart=None, departLane='first', departPos='base', departSpeed='0', arrivalLane='current', arrivalPos='max', arrivalSpeed='current', fromTaz='', toTaz='', line='', personCapacity=0, personNumber=0):
        ...
        """
        if v.initial_lane_id == "0to1_0":
                i = 0
        elif v.initial_lane_id == "0to1_1":
                i = 1
        elif v.initial_lane_id == "0to1_2":
                i = 2
        traci.vehicle.add(v.id, v.routeID, typeID=typeID, departSpeed=v.initial_speed, departLane=i, departPos=v.initial_position)
