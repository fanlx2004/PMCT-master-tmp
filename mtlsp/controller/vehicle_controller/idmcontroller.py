...
CAV_DESIRED_VELOCITY = 34.5 # 33.33 #35 # [m/s]
CAV_DELTA = 4.0  # []

# Lateral policy parameters
CAV_POLITENESS = 1  # in [0, 1]
CAV_LANE_CHANGE_MIN_ACC_GAIN = 0  # [m/s2]
CAV_LANE_CHANGE_MAX_BRAKING_IMPOSED = 10  # [m/s2]
Surrogate_LANE_CHANGE_MAX_BRAKING_IMPOSED = 10  # [m/s2]
...
class IDMController(BaseController):
    ...
    '''   @staticmethod
    def safety_layer_decision(observation, action):

        point = [observation["Ego"]["lane_index"], observation["Ego"]["velocity"]]
        if observation["Lead"] != None:
            point.append(observation["Lead"]["position"][0] - observation["Ego"]["position"][0])
            point.append(observation["Lead"]["velocity"])
        else:
            point.append(None)
            point.append(None)
        if observation["Foll"] != None:
            point.append(observation["Foll"]["position"][0] - observation["Ego"]["position"][0])
            point.append(observation["Foll"]["velocity"])
        else:
            point.append(None)
            point.append(None)
        if observation["LeftLead"] != None:
            point.append(observation["LeftLead"]["position"][0] - observation["Ego"]["position"][0])
            point.append(observation["LeftLead"]["velocity"])
        else:
            point.append(None)
            point.append(None)
        if observation["LeftFoll"] != None:
            point.append(observation["LeftFoll"]["position"][0] - observation["Ego"]["position"][0])
            point.append(observation["LeftFoll"]["velocity"])
        else:
            point.append(None)
            point.append(None)
        if observation["RightLead"] != None:
            point.append(observation["RightLead"]["position"][0] - observation["Ego"]["position"][0])
            point.append(observation["RightLead"]["velocity"])
        else:
            point.append(None)
            point.append(None)
        if observation["RightFoll"] != None:
            point.append(observation["RightFoll"]["position"][0] - observation["Ego"]["position"][0])
            point.append(observation["RightFoll"]["velocity"])
        else:
            point.append(None)
            point.append(None)
        
        result = {}
        q = get_q_value_new(point)
        
        alpha = 0.0
        a = action["longitudinal"]
        if q <= 4:
            if action["lateral"] == "left":
                result["lateral"] = "right"
                result["longitudinal"] = 0
                return result
            elif action["lateral"] == "right":
                result["lateral"] = "left"
                result["longitudinal"] = 0
                ...
                result["lateral"] = "central"
            alpha = 0.0
        elif q <= 9:
            if action["lateral"] == "left" or action["lateral"] == "right":
                return action
            result["lateral"] = "central"
            alpha = 0.25 * (q - 3) / (8 - 3)  
        else:
        ...
        beta = -4 * math.exp(-0.5 * (q - 1))

        result["longitudinal"] = max(min(a * alpha + beta, 2), -4) 
        return result'''
