...
class Simulator(object):
    ...
    def run(self):
        ...
        observation_dict_total = {}
        
        self.start()
        for lll in range(int(math.ceil(self.env.run_time / utils.get_step_size()))):
            self.step()
            observation_dict = {}
            for veh in self.env.vehicle_list.values():
                veh_lane = traci.vehicle.getLaneID(veh.id)
                if veh_lane.startswith(":"):
                    observation_dict[veh.id] = veh.observation["local_node"]
                else:
                    observation_dict[veh.id] = veh.observation["local"]
            observation_dict_total[str(traci.simulation.getTime())] = observation_dict
            if self.running is False:
                break
        self.stop()

        collisions = self._parse_collisions()
        ...
        for collision in collisions:
            ...
            related_obs = {}
            start_time = max(0, collision_time - 10.0)
            
            for time_str, obs_at_time in observation_dict_total.items():
                obs_time = float(time_str)
                
                if start_time <= obs_time <= collision_time:

                    filtered_obs = {}
                    for veh_id in [collider, victim]:
                        k_list = obs_at_time.keys()
                        if veh_id in k_list:
                            filtered_obs[veh_id] = obs_at_time[veh_id]
                    
                    if filtered_obs:
                        related_obs[time_str] = filtered_obs
                    else:
                        related_obs[time_str] = None
            
            collision_data = {
                "time": collision_time,
                "vehicles": [collider, victim],
                "data": related_obs
            }
            
            collision_data_list.append(collision_data)

        with open(os.path.join(self.output_path, "collision_analysis.json"), 'w') as f:
            json.dump(collision_data_list, f, indent=2)
    ...
    def _parse_collisions(self):
        ...
        collision_file = os.path.join(self.output_path, "collision.xml")
        ...
        try:
            ...
            start_idx = content.find('<collisions')
            if start_idx == -1:
                return collisions
            
            end_idx = content.find('</collisions>')
            if end_idx == -1:
                return collisions
            
            xml_content = content[start_idx:end_idx + len('</collisions>')]
            ...
            for elem in root.findall('collision'):
                try:
                    collision = {
                        ...
                        "collider_type": elem.get('colliderType', ''),
                        "victim_type": elem.get('victimType', ''),
                        "collider_speed": float(elem.get('colliderSpeed', 0)),
                        "victim_speed": float(elem.get('victimSpeed', 0))
                    }
    ...
    def _parse_collisions_simple(self, collision_file):
        collisions = []
            ...
            for match in matches:
                ...
                if attrs:
                    ...
                    for key, value in attrs:
                        collision_data[key] = value
                    
                    try:
                        collision = {
                            "time": float(collision_data.get('time', 0)),
                            "type": collision_data.get('type', ''),
                            "lane": collision_data.get('lane', ''),
                            "pos": float(collision_data.get('pos', 0)),
                            "collider": collision_data.get('collider', ''),
                            "victim": collision_data.get('victim', ''),
                            "collider_type": collision_data.get('colliderType', ''),
                            "victim_type": collision_data.get('victimType', ''),
                            "collider_speed": float(collision_data.get('colliderSpeed', 0)),
                            "victim_speed": float(collision_data.get('victimSpeed', 0))
                        }
                        collisions.append(collision)
                    except Exception as e:
                        continue
